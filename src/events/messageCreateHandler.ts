...
async function sendReply(
    ...
    embedColor: string,
    commandImage: string,
    versionButtonRow?: ActionRowBuilder<ButtonBuilder>,
...
export default event(Events.MessageCreate, async ({ client }, message) => {
    ...
    if (inMemoryCache && content.startsWith(constantsConfig.prefixCommandPrefix)) {
        ...
        if (commandTextMatch) {
            ...
            const commandCachedVersion = await inMemoryCache.get(
                `${MemoryCachePrefix.VERSION}:${commandText.toLowerCase()}`,
            );
            ...
            if (commandVersionName === 'GENERIC' && !commandVersionExplicitGeneric) {
                ...
                if (channelDefaultVersionCached) {
                    ...
                    ({
                        id: commandVersionId,
                        name: commandVersionName,
                        enabled: commandVersionEnabled,
                    } = channelDefaultVersion);
            ...
            const cachedCommandDetails = await inMemoryCache.get(
                `${MemoryCachePrefix.COMMAND}:${commandText.toLowerCase()}`,
            );
            if (cachedCommandDetails) {
                ...
                if (!botPermissions || !botPermissions.has(requiredBotPermissions)) {
                    Logger.info(
                        `Bot does not have required permissions in channel ${channelName} (${channelId}) of server ${guildId}. Unable to process message ${messageId} from user ${authorId}.`,
                    );
                    return;
                }
                ...
                Logger.info(
                    `Prefix Command - Executing command "${name}" and version "${commandVersionName}" based on user command "${commandText}" in channel ${channelName}.`,
                );
                ...
                if (enableButtons && commandVersionName === 'GENERIC' && contents.length > 1) {
                    ...
                    for (const { versionId: versionIdForButton } of contents) {
                        const versionCached = await inMemoryCache.get(
                            `${MemoryCachePrefix.VERSION}:${versionIdForButton}`,
                        );
                        if (versionCached) {
                    ...
                    const versionSelectButtonRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
                    ...
                    collector.on('collect', async (collectedInteraction: ButtonInteraction) => {
                        ...
                        Logger.debug(
                            `Prefix Command - User selected button "${collectedInteraction.customId}" for command "${name}" based on user command "${commandText}"`,
                        );
                        try {
