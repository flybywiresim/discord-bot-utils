 ...
 */
export const fetchData = async <ReturnType>(request: Request, typeGuard: TypeGuard<ReturnType>): Promise<ReturnType> => {
    try {
        ...
        if (!typeGuard(data)) {
            return Promise.reject(new Error('Type guard not satisfied.'));
        }

        return Promise.resolve<ReturnType>(data as ReturnType);
    } catch (e) {
